  ...
  return (
    <>
      <div ref={controlsRef} className="relative flex shrink-0 items-center gap-3 sm:gap-5">
        ...
        >
          <Bell size={20} />
          <span className="absolute right-1.5 top-1.5 h-2 w-2 rounded-full bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.6)]" />
        </button>
        ...
        {menuOpen && (
          <div className="absolute right-0 top-[calc(100%+10px)] z-[70] w-64 overflow-hidden rounded-xl border shadow-[0_18px_50px_rgba(0,0,0,0.5)] transition-colors duration-300 bg-white border-slate-200 dark:bg-[#111a29] dark:border-slate-700 midnight:bg-[#0a1120] midnight:border-cyan-900/50">
            ...
            <button type="button" onClick={openNotifications} className="mt-2 flex w-full items-center justify-between rounded-lg px-3 py-2.5 text-sm transition-colors duration-200 text-slate-600 hover:bg-slate-100 hover:text-slate-900 dark:text-slate-300 dark:hover:bg-white/5 dark:hover:text-white midnight:text-cyan-100/70 midnight:hover:bg-cyan-900/30 midnight:hover:text-cyan-50">
              ...
              <span className="rounded-full bg-red-500 px-1.5 py-0.5 text-[9px] font-bold text-white shadow-sm">3</span>
            </button>
        ...
        {/* MENÚ DESPLEGABLE DE NOTIFICACIONESs */}
        {notificationsOpen && (
          <section className="absolute right-0 top-[calc(100%+10px)] z-[70] w-[min(330px,calc(100vw-24px))] overflow-hidden rounded-xl border shadow-[0_18px_50px_rgba(0,0,0,0.5)] transition-colors duration-300 bg-white border-slate-200 dark:bg-[#111a29] dark:border-slate-700 midnight:bg-[#0a1120] midnight:border-cyan-900/50">
            <header className="flex items-center justify-between border-b px-4 py-3 border-slate-100 dark:border-slate-800 midnight:border-cyan-900/30">
              ...
              <span className="text-[10px] font-medium text-cyan-600 dark:text-cyan-400 midnight:text-cyan-400">3 nuevas</span>
            </header>
            <div className="divide-y divide-slate-100 dark:divide-slate-800 midnight:divide-cyan-900/30">
              {notifications.map((notification) => (
                <article key={notification.id} className="px-4 py-3 transition-colors duration-200 hover:bg-slate-50 dark:hover:bg-white/[0.03] midnight:hover:bg-cyan-900/20 cursor-pointer">
                  <div className="flex gap-3">
                    <span className="mt-1 h-2 w-2 shrink-0 rounded-full bg-cyan-500 dark:bg-cyan-400 midnight:bg-cyan-400 shadow-[0_0_8px_rgba(6,182,212,0.5)]" />
                    <div>
                      <h3 className="text-xs font-semibold text-slate-800 dark:text-slate-200 midnight:text-cyan-100">{notification.title}</h3>
                      <p className="mt-1 text-[11px] leading-relaxed text-slate-500 dark:text-slate-400 midnight:text-cyan-500/70">{notification.detail}</p>
                      <span className="mt-1.5 block text-[9px] text-slate-400 dark:text-slate-600 midnight:text-cyan-500/50">{notification.time}</span>
                    </div>
                  </div>
                </article>
              ))}
